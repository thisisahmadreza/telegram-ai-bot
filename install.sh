...
cat > deploy/telegram-ai-bot.service << 'EOT'
[Unit]
...
ExecStart=/home/ubuntu/telegram-ai-bot/.venv/bin/python -m bot.main
Restart=always
RestartSec=5
[Install]
...
cat > scripts/setup_ubuntu24.sh << 'EOT'
#!/usr/bin/env bash
set -euo pipefail
sudo apt update
...
EOT
chmod +x scripts/setup_ubuntu24.sh

echo "Installed project files."
echo "Next:"
echo "1) cp .env.example .env && nano .env"
echo "2) ./scripts/setup_ubuntu24.sh"
echo "3) source .venv/bin/activate && python -m bot.main"
EOF

chmod +x install.sh
git add install.sh
git commit -m "Add one-command installer"
